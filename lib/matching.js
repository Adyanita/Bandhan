...
export function getAge(dob) {
  if (!dob) return NaN;
  const born = new Date(dob).getFullYear();
 ...
 * @param {object | null | undefined} viewerProfile - Current user's Profile row (not User row)
 */
export function applyFilters(profiles, viewerProfile, filters, searchQuery) {
  return profiles.filter((p) => {
    if (!p.dob) return false;
    if (viewerProfile?.id && p.id === viewerProfile.id) return false;
    if (viewerProfile?.gender && p.gender === viewerProfile.gender) return false;
